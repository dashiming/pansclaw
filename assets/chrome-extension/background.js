...
const BADGE = {
  on: { text: 'ON', color: '#2563EB' },
  off: { text: '', color: '#000000' },
  connecting: { text: '…', color: '#60A5FA' },
  error: { text: '!', color: '#1D4ED8' },
}
...
async function ensureRelayConnection() {
  ...
  relayConnectPromise = (async () => {
    ...
    const wsUrls = await buildRelayWsUrls(port, gatewayToken)
    ...
    throw lastError instanceof Error ? lastError : new Error('WebSocket connect failed')
  })()
  ...
  for (const [tabId, tab] of tabs.entries()) {
    if (tab.state === 'connected') {
      ...
      void chrome.action.setTitle({
        tabId,
        title: 'PansClaw Browser Relay: relay reconnecting…',
      })
    }
  }
...
async function reannounceAttachedTabs() {
  for (const [tabId, tab] of tabs.entries()) {
    ...
    if (!valid) {
      ...
      void chrome.action.setTitle({
        tabId,
        title: 'PansClaw Browser Relay (click to attach/detach)',
      })
      continue
    }
    ...
    try {
      ...
      void chrome.action.setTitle({
        tabId,
        title: 'PansClaw Browser Relay: attached (click to detach)',
      })
    } catch {
...
async function maybeOpenHelpOnce(reason = 'relay-unreachable') {
  try {
    const stored = await chrome.storage.local.get(['helpOnErrorShown'])
    if (stored.helpOnErrorShown === true) return
    await chrome.storage.local.set({ helpOnErrorShown: true, setupNotice: reason })
...
async function connectOrToggleForActiveTab() {
  ...
  try {
    ...
    void chrome.action.setTitle({
      tabId,
      title: 'PansClaw Browser Relay: connecting to local relay…',
    })
    ...
    try {
      ...
      const helpReason = normalized.includes('missing gatewaytoken')
        ? 'missing-token'
        : normalized.includes('connect failed') || normalized.includes('gateway connect failed')
          ? 'auth-failed'
          : 'relay-unreachable'
      const title = helpReason === 'missing-token'
        ? 'PansClaw Browser Relay: gateway token required (open options for setup)'
        : helpReason === 'auth-failed'
          ? 'PansClaw Browser Relay: token rejected (open options for setup)'
          : 'PansClaw Browser Relay: relay not running (open options for setup)'
      void chrome.action.setTitle({
        tabId,
        title,
      })
      void maybeOpenHelpOnce(helpReason)
...
async function onDebuggerDetach(source, reason) {
  ...
  void chrome.action.setTitle({
    tabId,
    title: 'PansClaw Browser Relay: re-attaching after navigation…',
  })
  ...
  for (let attempt = 0; attempt < delays.length; attempt++) {
    ...
    try {
      ...
      if (!relayUp) {
        ...
        void chrome.action.setTitle({
          tabId,
          title: 'PansClaw Browser Relay: attached, waiting for relay reconnect…',
        })
      }
      return
    } catch {
  ...
  void chrome.action.setTitle({
    tabId,
    title: 'PansClaw Browser Relay: re-attach failed (click to retry)',
  })
}
