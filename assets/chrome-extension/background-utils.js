...
export async function buildRelayWsUrls(port, gatewayToken) {
  const token = String(gatewayToken || "").trim();
  if (!token) {
    throw new Error(
      "Missing gatewayToken in extension settings (chrome.storage.local.gatewayToken)",
  ...
  const relayToken = await deriveRelayToken(token, port);
  const urls = [
    `ws://127.0.0.1:${port}/extension?token=${encodeURIComponent(relayToken)}`,
  ];

  // Older PansClaw relays accept the raw gateway token directly.
  if (relayToken !== token) {
    urls.push(`ws://127.0.0.1:${port}/extension?token=${encodeURIComponent(token)}`);
  }
