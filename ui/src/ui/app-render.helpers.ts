...
function resetChatStateForSessionSwitch(state: AppViewState, sessionKey: string) {
  ...
  (state as unknown as PansClawApp).chatStreamStartedAt = null;
  state.chatRunId = null;
  (state as unknown as PansClawApp).resetToolStream();
  (state as unknown as PansClawApp).resetChatScroll();
  state.applySettings({
    ...state.settings,
    sessionKey,
...
export function renderChatSessionSelect(state: AppViewState) {
  ...
  return html`
    <style>
      ...
      .chat-controls__session-row .chat-controls__session {
        min-width: 260px;
      }
  ...
  return html`
    <div class="chat-controls">
      <button
        ...
        @click=${async () => {
          const app = state as unknown as PansClawApp;
