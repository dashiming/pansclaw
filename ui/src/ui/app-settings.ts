...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "skills") {
    await loadSkills(host as unknown as PansClawApp);
  }
  if (host.tab === "agents") {
    await loadAgents(host as unknown as PansClawApp);
    await loadConfig(host as unknown as PansClawApp);
    const agentIds = host.agentsList?.agents?.map((entry) => entry.id) ?? [];
    if (agentIds.length > 0) {
      void loadAgentIdentities(host as unknown as PansClawApp, agentIds);
    }
    const agentId =
      host.agentsSelectedId ?? host.agentsList?.defaultId ?? host.agentsList?.agents?.[0]?.id;
    if (agentId) {
      void loadAgentIdentity(host as unknown as PansClawApp, agentId);
      if (host.agentsPanel === "skills") {
        void loadAgentSkills(host as unknown as PansClawApp, agentId);
      }
      if (host.agentsPanel === "channels") {
        void loadChannels(host as unknown as PansClawApp, false);
      }
  ...
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as PansClawApp);
    await loadDevices(host as unknown as PansClawApp);
    await loadConfig(host as unknown as PansClawApp);
    await loadExecApprovals(host as unknown as PansClawApp);
  }
  if (host.tab === "chat") {
  ...
  if (
    ...
    await loadConfigSchema(host as unknown as PansClawApp);
    await loadConfig(host as unknown as PansClawApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as PansClawApp);
...
export async function loadOverview(host: SettingsHost) {
  const app = host as unknown as PansClawApp;
...
async function loadOverviewLogs(host: PansClawApp) {
...
function buildAttentionItems(host: PansClawApp) {
...
export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as PansClawApp, true),
    loadConfigSchema(host as unknown as PansClawApp),
    loadConfig(host as unknown as PansClawApp),
  ]);
}
