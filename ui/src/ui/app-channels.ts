...
export async function handleWhatsAppStart(host: PansClawApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: PansClawApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: PansClawApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: PansClawApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: PansClawApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: PansClawApp): string {
...
function resolveGatewayHttpAuthHeader(host: PansClawApp): string | null {
...
function buildGatewayHttpHeaders(host: PansClawApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: PansClawApp) {
...
export function handleNostrProfileToggleAdvanced(host: PansClawApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: PansClawApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: PansClawApp) {
  const state = host.nostrProfileFormState;
