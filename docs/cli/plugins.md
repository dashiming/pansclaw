...
Bundled plugins ship with PansClaw but start disabled. Use `plugins enable` to
activate them.

Native PansClaw plugins must ship `openclaw.plugin.json` with an inline JSON
...
If a bare install spec matches a bundled plugin id (for example `diffs`), PansClaw
installs the bundled plugin directly. To install an npm package with the same
name, use an explicit scoped spec (for example `@scope/diffs`).

Supported archives: `.zip`, `.tgz`, `.tar.gz`, `.tar`.

For local paths and archives, PansClaw auto-detects:

- native PansClaw plugins (`openclaw.plugin.json`)
...
PansClaw prints a warning and asks for confirmation before proceeding. Use
