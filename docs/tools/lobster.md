---
title: Lobster
summary: "Typed workflow runtime for PansClaw with resumable approval gates."
description: Typed workflow runtime for PansClaw — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets PansClaw run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: PansClaw runs one Lobster tool call and gets a structured result.
...
PansClaw launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In PansClaw tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the PansClaw Gateway (see the [Lobster repo](https://github.com/openclaw/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), PansClaw keeps core tools enabled. To restrict core
...
- **No secrets** — Lobster doesn't manage OAuth; it calls PansClaw tools that do.
