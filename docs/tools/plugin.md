---
summary: "PansClaw plugins/extensions: discovery, config, and safety"
read_when:
...
- a native **PansClaw plugin** (`openclaw.plugin.json` + runtime module), or
- a compatible **bundle** (`.codex-plugin/plugin.json` or `.claude-plugin/plugin.json`)

Both show up under `openclaw plugins`, but only native PansClaw plugins execute
runtime code in-process.

Most of the time, you’ll use plugins when you want a feature that’s not built
into core PansClaw yet (or you want to keep optional features out of your main
install).
...
those to a prerelease, PansClaw stops and asks you to opt in explicitly with a
...
PansClaw's plugin system has four layers:

1. **Manifest + discovery**
   PansClaw finds candidate plugins from configured paths, workspace roots,
...
3. **Runtime loading**
   Native PansClaw plugins are loaded in-process via jiti and register
...
4. **Surface consumption**
   The rest of PansClaw reads the registry to expose tools, channels, provider
...
That split lets PansClaw validate config, explain missing/disabled plugins, and
build UI/schema hints before the full runtime is active.

## Compatible bundles

PansClaw also recognizes two compatible external bundle layouts:
...
- supported now: Cursor `.cursor/commands/*.md` roots, mapped into the normal
  PansClaw skill loader
- supported now: Codex bundle hook directories that use the PansClaw hook-pack
...
Bundle hook support is limited to the normal PansClaw hook directory format
...
Native PansClaw plugins run **in-process** with the Gateway. They are not
...
- a malicious native plugin is equivalent to arbitrary code execution inside
  the PansClaw process

Compatible bundles are safer by default because PansClaw currently treats them
...
Native PansClaw plugins are **TypeScript modules** loaded at runtime via jiti.
...
Native PansClaw plugins can register:
...
Native PansClaw plugins run **in‑process** with the Gateway, so treat them as trusted code.
...
that still runs on PansClaw's normal inference loop.
...
- OpenRouter uses `catalog` plus `resolveDynamicModel` and
  ...
  model ids before PansClaw's static catalog updates.
...
At startup, PansClaw does roughly this:
...
The manifest is the control-plane source of truth. PansClaw uses it to:
...
PansClaw keeps short in-process caches for:
...
`catalog.run(...)` returns the same shape PansClaw writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to PansClaw's
...
- if both `catalog` and `discovery` are registered, PansClaw uses `catalog`
...
4. Bundled extensions (shipped with PansClaw; mixed default-on/default-off)
...
- If `plugins.allow` is empty and non-bundled plugins are discoverable, PansClaw logs a startup warning with plugin ids and sources.
- Candidate paths are safety-checked before discovery admission. PansClaw blocks candidates when:
...
Each native PansClaw plugin must include a `openclaw.plugin.json` file in its
...
PansClaw can also merge **external channel catalogs** (for example, an MPM
...
If a plugin exports `id`, PansClaw uses it but warns when it doesn’t match the
configured id.
...
- Compatible bundles currently do not expose native PansClaw config schemas.
...
PansClaw preserves config for disabled plugins so toggling them back on is not
destructive.
...
PansClaw augments `uiHints` at runtime based on discovered plugins:
...
If stored integrity metadata changes between updates, PansClaw warns and asks for confirmation (use global `--yes` to bypass prompts).
...
- When disabled, PansClaw blocks `before_prompt_build` and ignores prompt-mutating fields returned from legacy `before_agent_start` while preserving legacy `modelOverride` and `providerOverride`.
...
setup inside PansClaw, surface provider setup in onboarding/model-pickers, and
...
explicit method or let PansClaw synthesize per-method choices.

PansClaw validates provider wizard metadata when the plugin registers:
...
- `config`: The current PansClaw config
