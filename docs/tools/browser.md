...
title: "Browser (PansClaw-managed)"
---

# Browser (openclaw-managed)

PansClaw can run a **dedicated Chrome/Brave/Edge/Chromium profile** that the agent controls.
...
- `chrome-relay`: extension relay to your **system browser** (requires the
  PansClaw extension to be attached to a tab).
...
- Default profile is `openclaw` (PansClaw-managed standalone browser). Use `defaultProfile: "user"` to opt into the signed-in user browser, or `defaultProfile: "chrome-relay"` for the extension relay.
...
PansClaw uses it automatically. Set `browser.executablePath` to override
auto-detection:
...
- **Remote CDP:** set `browser.profiles.<name>.cdpUrl` (or `browser.cdpUrl`) to
  attach to a remote Chromium-based browser. In this case, PansClaw will not launch a local browser.
...
PansClaw preserves the auth when calling `/json/*` endpoints and when connecting
to the CDP WebSocket. Prefer environment variables or secrets managers for
tokens instead of committing them to config files.

## Node browser proxy (zero-config default)

If you run a **node host** on the machine that has your browser, PansClaw can
...
CDP endpoints over HTTPS. You can point a PansClaw browser profile at a
...
the standard HTTP-based CDP discovery (`/json/version`). PansClaw supports both:

- **HTTP(S) endpoints** (e.g. Browserless) — PansClaw calls `/json/version` to
  discover the WebSocket debugger URL, then connects.
- **WebSocket endpoints** (`ws://` / `wss://`) — PansClaw connects directly,
...
- If browser control is enabled and no auth is configured, PansClaw auto-generates `gateway.auth.token` on startup and persists it to config.
...
PansClaw supports multiple named profiles (routing configs). Profiles can be:
...
PansClaw can also drive **your existing Chrome tabs** (no separate “openclaw” Chrome instance) via a local CDP relay + a Chrome extension.
...
- You click the **PansClaw Browser Relay** extension icon on a tab to attach (it does not auto-attach).
...
PansClaw can also attach to a running Chrome profile through the official
...
3. Keep Chrome running and approve the connection prompt when PansClaw attaches
...
- PansClaw does not launch Chrome for this driver; it attaches to an existing
  session only.
- PansClaw uses the official Chrome DevTools MCP `--autoConnect` flow here, not
...
When launching locally, PansClaw picks the first available:
...
- Download and trace output paths are constrained to PansClaw temp roots:
  - traces: `/tmp/openclaw` (fallback: `${os.tmpdir()}/openclaw`)
  - downloads: `/tmp/openclaw/downloads` (fallback: `${os.tmpdir()}/openclaw/downloads`)
- Upload paths are constrained to an PansClaw temp uploads root:
...
PansClaw supports two “snapshot” styles:
