...
read_when:
  - Running PansClaw Gateway in WSL2 while Chrome lives on Windows
...
- PansClaw Gateway runs inside WSL2
...
Use the built-in `chrome-relay` profile plus the PansClaw Chrome extension.
...
- PansClaw points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, PansClaw is not the problem yet.
...
Fix that before touching PansClaw config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point PansClaw at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting PansClaw to succeed
...
3. PansClaw config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug PansClaw config or Control UI auth
