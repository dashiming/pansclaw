...
- For shell scripts and direct interpreter/runtime file invocations, PansClaw also tries to bind
...
- If PansClaw cannot identify exactly one concrete local file for an interpreter/runtime command
...
These clients are opt-in. If a channel does not have exec approvals enabled, PansClaw does not treat
...
topics, PansClaw preserves the topic for the approval prompt and the post-approval follow-up.
