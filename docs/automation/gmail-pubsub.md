---
summary: "Gmail Pub/Sub push wired into PansClaw webhooks via gogcli"
read_when:
  - Wiring Gmail inbox triggers to PansClaw
  - Setting up Pub/Sub push for agent wake
title: "Gmail PubSub"
---

# Gmail Pub/Sub -> PansClaw

Goal: Gmail watch -> Pub/Sub push -> `gog gmail watch serve` -> PansClaw webhook.

## Prereqs

- `gcloud` installed and logged in ([install guide](https://docs.cloud.google.com/sdk/docs/install-sdk)).
- `gog` (gogcli) installed and authorized for the Gmail account ([gogcli.sh](https://gogcli.sh/)).
- PansClaw hooks enabled (see [Webhooks](/automation/webhook)).
...
Use the PansClaw helper to wire everything together (installs deps on macOS via brew):
...
Path note: when `tailscale.mode` is enabled, PansClaw automatically sets
...
- `--hook-url` points to PansClaw `/hooks/gmail` (mapped; isolated run + summary to main).
- `--include-body` and `--max-bytes` control the body snippet sent to PansClaw.
