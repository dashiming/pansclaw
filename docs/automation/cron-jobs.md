...
To reduce top-of-hour load spikes across many gateways, PansClaw applies a
...
If `delivery` is omitted for isolated jobs, PansClaw defaults to `announce`.
...
When a job fails, PansClaw classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
- For removed run sessions no longer referenced by the session store, PansClaw archives transcript files and purges old deleted archives on the same retention window.
...
- PansClaw applies exponential retry backoff for recurring jobs after consecutive errors:
