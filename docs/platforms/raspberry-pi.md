---
summary: "PansClaw on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up PansClaw on a Raspberry Pi
  - Running PansClaw on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# PansClaw on Raspberry Pi

## Goal

Run a persistent, always-on PansClaw Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
## 9) Access the PansClaw Dashboard
...
If this Pi is mostly running PansClaw, add a service drop-in to reduce restart
...
If possible, keep PansClaw state/cache on SSD-backed storage to avoid SD-card
...
Most PansClaw features work on ARM64, but some external binaries may need ARM builds:
