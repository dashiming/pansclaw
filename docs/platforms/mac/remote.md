---
summary: "macOS app flow for controlling a remote PansClaw gateway over SSH"
read_when:
  - Setting up or debugging remote mac control
title: "Remote Control"
---

# Remote PansClaw (macOS ⇄ remote host)

This flow lets the macOS app act as a full remote control for a PansClaw gateway running on another host (desktop/server). It’s the app’s **Remote over SSH** (remote run) feature. All features—health checks, Voice Wake forwarding, and Web Chat—reuse the same remote SSH configuration from _Settings → General_.

## Modes

- **Local (this Mac)**: Everything runs on the laptop. No SSH involved.
- **Remote over SSH (default)**: PansClaw commands are executed on the remote host. The mac app opens an SSH connection with `-o BatchMode` plus your chosen identity/key and a local port-forward.
...
1. Install Node + pnpm and build/install the PansClaw CLI (`pnpm install && pnpm build && pnpm link --global`).
...
2. Under **PansClaw runs**, pick **Remote over SSH** and set:
