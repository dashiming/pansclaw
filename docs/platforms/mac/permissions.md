...
- Same path: run the app from a fixed location (for PansClaw, `dist/PansClaw.app`).
...
Workaround: move files into the PansClaw workspace (`~/.openclaw/workspace`) if you want to avoid per-folder grants.
