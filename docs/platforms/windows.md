...
read_when:
  - Installing PansClaw on Windows
...
PansClaw on Windows is recommended **via WSL2** (Ubuntu recommended). The
...
- if Scheduled Task creation is denied, PansClaw falls back to a per-user Startup-folder login item and starts the gateway immediately
- if `schtasks` itself wedges or stops responding, PansClaw now aborts that path quickly and falls back instead of hanging forever
...
### 2) Install the PansClaw gateway user service
