---
summary: "Use Synthetic's Anthropic-compatible API in PansClaw"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. PansClaw registers it as the
...
Note: PansClaw's Anthropic client appends `/v1` to the base URL, so use
