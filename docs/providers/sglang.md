---
summary: "Run PansClaw with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run PansClaw against a local SGLang server
...
PansClaw can connect to SGLang using the `openai-completions` API.

PansClaw can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, PansClaw will query:
