...
Accessed via `PansClawPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: PansClawConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: PansClawConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: PansClawConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: PansClawConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: PansClawConfig): string;
  };
};
```
...
- Add `api.runtime` to `PansClawPluginApi` with the surface above.
