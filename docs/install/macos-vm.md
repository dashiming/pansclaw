---
summary: "Run PansClaw in a sandboxed macOS VM (local or hosted) when you need isolation or iMessage"
read_when:
  - You want PansClaw isolated from your main macOS environment
...
# PansClaw on macOS VMs (Sandboxing)
...
Run PansClaw in a sandboxed macOS VM on your existing Apple Silicon Mac using [Lume](https://cua.ai/docs/lume).
...
5. SSH in, install PansClaw, configure channels
6. Done
...
The VM runs in the background. PansClaw's daemon keeps the gateway running.
...
This is the killer feature of running on macOS. Use [BlueBubbles](https://bluebubbles.app) to add iMessage to PansClaw.
