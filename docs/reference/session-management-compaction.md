...
This document explains how PansClaw manages sessions end-to-end:
...
PansClaw is designed around a single **Gateway process** that owns session state.
...
PansClaw persists sessions in two layers:
...
PansClaw resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, PansClaw reports potential evictions but does not mutate the store/files.
...
PansClaw intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (PansClaw consumes the events, but Pi decides when to compact).
...
PansClaw also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, PansClaw bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, PansClaw leaves it alone.
...
PansClaw supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- PansClaw strips/suppresses this in the delivery layer.

As of `2026.1.10`, PansClaw also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
PansClaw uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but PansClaw’s
