...
- With Anthropic API-key auth profiles, PansClaw seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, PansClaw injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
PansClaw exposes dedicated cache-trace diagnostics for embedded agent runs.
