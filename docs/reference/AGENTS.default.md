...
summary: "Default PansClaw agent instructions and skills roster for the personal assistant setup"
read_when:
  - Starting a new PansClaw agent session
  - Enabling or auditing default skills
---

# AGENTS.md — PansClaw Personal Assistant (default)

## First run (recommended)

PansClaw uses a dedicated workspace directory for the agent. Default: `~/.openclaw/workspace` (configurable via `agents.defaults.workspace`).
...
- For browser-driven verification, use `openclaw browser` (tabs/status/screenshot) with the PansClaw-managed Chrome profile.
