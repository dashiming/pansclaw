...
```
  ...
  <Tab title="HTTP Events API mode">
    <Steps>
      ...
      <Step title="Configure PansClaw HTTP mode">
...
- The interactive callback values are PansClaw-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, PansClaw falls back to the original text reply instead of sending an invalid blocks payload.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while PansClaw is processing a reply, then removes it when the run finishes. This is a useful fallback when Slack native assistant typing is unavailable, especially in DMs.
...
```json
{
  "display_information": {
    "name": "PansClaw",
    "description": "Slack connector for PansClaw"
  },
  "features": {
    "bot_user": {
      "display_name": "PansClaw",
      "always_online": false
    },
    "app_home": {
    ...
    "slash_commands": [
      {
        "command": "/openclaw",
        "description": "Send a message to PansClaw",
        "should_escape": false
      }
    ]
  },
...
PansClaw supports Slack native text streaming via the Agents and AI Apps API.
...
- If streaming fails mid-reply, PansClaw falls back to normal delivery for remaining payloads.
