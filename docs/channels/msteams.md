...
3. Configure PansClaw with those credentials.
...
- Talk to PansClaw via Teams DMs, group chats, or channels.
...
- On startup, PansClaw resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
Before configuring PansClaw, you need to create an Azure Bot resource.
...
```json
{
  ...
  "description": { "short": "PansClaw in Teams", "full": "PansClaw in Teams" },
...
PansClaw handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, PansClaw only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/PansClawShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

PansClaw sends Teams polls as Adaptive Cards (there is no native Teams poll API).
