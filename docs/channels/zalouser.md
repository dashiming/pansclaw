...
read_when:
  - Setting up Zalo Personal for PansClaw
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside PansClaw.
...
- On startup, PansClaw resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, PansClaw stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in PansClaw state. Example:
...
- PansClaw sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, PansClaw sends delivered + seen acknowledgements (best-effort).
...
- The channel now runs fully in PansClaw without external CLI binaries.
