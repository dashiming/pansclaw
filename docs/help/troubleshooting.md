---
summary: "Symptom first troubleshooting hub for PansClaw"
read_when:
  - PansClaw is not working and you need the fastest path to a fix
...
is using an old shape that PansClaw no longer accepts.
...
```mermaid
flowchart TD
  A[PansClaw is not working] --> B{What breaks first}
  B --> C[No replies]
