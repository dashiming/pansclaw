---
summary: "Where PansClaw loads environment variables and the precedence order"
read_when:
...
PansClaw pulls environment variables from multiple sources. The rule is **never override existing values**.
...
PansClaw also injects context markers into spawned child processes:
...
- `COLORFGBG`: if your terminal exports it, PansClaw uses the background color hint to auto-pick the TUI palette.
...
PansClaw supports two env-driven patterns:
...
| Variable               | Purpose                                                                                                                                                                          |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `OPENCLAW_HOME`        | Override the home directory used for all internal path resolution (`~/.openclaw/`, agent dirs, sessions, credentials). Useful when running PansClaw as a dedicated service user. |
