...
read_when:
  - Setting up PansClaw for the first time
...
PansClaw reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.openclaw/openclaw.json`.

If the file is missing, PansClaw uses safe defaults. Common reasons to add a config:
...
<Warning>
PansClaw only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>
...
PansClaw reads env vars from the parent process plus:
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, PansClaw runs your login shell and imports only the missing keys:
