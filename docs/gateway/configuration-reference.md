...
summary: "Complete reference for every PansClaw config key, defaults, and channel settings"
read_when:
...
Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — PansClaw uses safe defaults when omitted.
...
- PansClaw additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
- `commands.callbackUrl` must resolve to the PansClaw gateway endpoint and be reachable from the Mattermost server.
...
PansClaw spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `openclaw channels add` (or channel onboarding) while still on a single-account top-level channel config, PansClaw moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
...
Optional repository root shown in the system prompt's Runtime line. If unset, PansClaw auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, PansClaw assumes `anthropic` (deprecated).
...
- `timeoutSeconds`: maximum seconds allowed for a single compaction operation before PansClaw aborts it. Default: `900`.
...
noVNC observer access uses VNC auth by default and PansClaw emits a short-lived token URL (instead of exposing the password in the shared URL).
...
For `type: "acp"` entries, PansClaw resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, PansClaw starts a fresh thread session instead of inheriting parent transcript history.
...
- When `openai.baseUrl` points to a non-OpenAI endpoint, PansClaw treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
PansClaw uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.openclaw/agents/<agentId>/agent/models.json`.
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), PansClaw forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
...
- Discovery accepts native PansClaw plugins plus compatible Codex bundles and Claude bundles, including manifestless Claude default-layout bundles.
- **Config changes require a gateway restart.**
- `allow`: optional allowlist (only listed plugins load). `deny` wins.
- `plugins.entries.<id>.apiKey`: plugin-level API key convenience field (when supported by the plugin).
- `plugins.entries.<id>.env`: plugin-scoped env var map.
- `plugins.entries.<id>.hooks.allowPromptInjection`: when `false`, core blocks `before_prompt_build` and ignores prompt-mutating fields from legacy `before_agent_start`, while preserving legacy `modelOverride` and `providerOverride`. Applies to native plugin hooks and supported bundle-provided hook directories.
- `plugins.entries.<id>.config`: plugin-defined config object (validated by native PansClaw plugin schema when available).
- Enabled Claude bundle plugins can also contribute embedded Pi defaults from `settings.json`; PansClaw applies those as sanitized agent settings, not as raw PansClaw config patches.
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one PansClaw.`).
