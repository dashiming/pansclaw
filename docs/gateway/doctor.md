...
`openclaw doctor` is the repair + migration tool for PansClaw. It fixes stale
...
offers to remove them and install the PansClaw service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named PansClaw gateway services are considered first-class and are not
flagged as "extra."
