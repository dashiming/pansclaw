...
- During heartbeat runs, PansClaw treats `HEARTBEAT_OK` as an ack when it appears
...
If **all three** are false, PansClaw skips the heartbeat run entirely (no model call).
