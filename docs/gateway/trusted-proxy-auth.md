...
read_when:
  - Running PansClaw behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of PansClaw
...
- You run PansClaw behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. PansClaw checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. PansClaw extracts the user identity from the configured header
...
- PansClaw can stay on loopback HTTP behind the proxy.
...
If PansClaw itself serves HTTPS directly (no TLS-terminating proxy), set:
...
3. Update PansClaw config with trusted-proxy auth
