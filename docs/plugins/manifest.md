...
read_when:
  - You are building a PansClaw plugin
...
This page is for the **native PansClaw plugin manifest** only.
...
PansClaw auto-detects those bundle layouts too, but they are not validated
against the `openclaw.plugin.json` schema described here.

For compatible bundles, PansClaw currently reads bundle metadata plus declared
skill roots, Claude command roots, Claude bundle `settings.json` defaults, and
supported hook packs when the layout matches PansClaw runtime expectations.

Every native PansClaw plugin **must** ship a `openclaw.plugin.json` file in the
**plugin root**. PansClaw uses this manifest to validate configuration
...
- The manifest is **required for native PansClaw plugins**, including local filesystem loads.
