---
summary: "Unified bundle format guide for Codex, Claude, and Cursor bundles in PansClaw"
read_when:
  - You want to install or debug a Codex, Claude, or Cursor-compatible bundle
  - You need to understand how PansClaw maps bundle content into native features
...
PansClaw supports one shared class of external plugin package: **bundle
plugins**.
...
PansClaw shows all of them as `Format: bundle` in `openclaw plugins list`.
...
A bundle is a **content/metadata pack**, not a native in-process PansClaw
plugin.

Today, PansClaw does **not** execute bundle runtime code in-process. Instead,
it detects known bundle files, reads the metadata, and maps supported bundle
content into native PansClaw surfaces such as skills, hook packs, MCP config,
and embedded Pi settings.

That is the main trust boundary:

- native PansClaw plugin: runtime module executes in-process
- bundle: metadata/content pack, with selective feature mapping

## Shared bundle model

Codex, Claude, and Cursor bundles are similar enough that PansClaw treats them
...
Common PansClaw behavior:

- detect the bundle subtype
- normalize it into one internal bundle record
- map supported parts into native PansClaw features
...
first. The more useful question is: which bundle surfaces does PansClaw map
today?

## Detection order

PansClaw prefers native PansClaw plugin/package layouts before bundle handling.
...
- package installs with valid `package.json` + `openclaw.extensions` use the
  native install path
- if a directory contains both native and bundle metadata, PansClaw treats it
  as native first
...
PansClaw normalizes bundle metadata into one internal bundle record, then maps
...
- bundle skill roots load as normal PansClaw skill roots
- Claude `commands` roots are treated as additional skill roots
- Cursor `.cursor/commands` roots are treated as additional skill roots

This means Claude markdown command files work through the normal PansClaw skill
loader. Cursor command markdown works through the same path.

#### Hook packs

- bundle hook roots work **only** when they use the normal PansClaw hook-pack
...
- PansClaw merges bundle MCP config into the backend `--mcp-config` file

#### Embedded Pi settings

- Claude `settings.json` is imported as default embedded Pi settings when the
  bundle is enabled
- PansClaw sanitizes shell override keys before applying them
...
diagnostics/info output, but PansClaw does not run them yet:
...
```text
bundle capability detected but not wired into PansClaw yet: agents
```
...
- bundle MCP is considered supported where PansClaw actually imports it
- Codex `hooks` is considered supported only for PansClaw hook-pack layouts

## Format differences

The formats are close, but not byte-for-byte identical. These are the practical
differences that matter in PansClaw.
...
Codex bundles fit PansClaw best when they use skill roots and PansClaw-style
...
Default Claude layout markers PansClaw recognizes:
...
Claude bundle manifests can declare custom component paths. PansClaw treats
...
- PansClaw does not execute arbitrary bundle runtime code in-process
...
If the directory is a native PansClaw plugin/package, the native install path
still wins.
...
If the capability is listed but PansClaw says it is not wired yet, that is a
...
PansClaw does not treat bundle settings as raw PansClaw config patches.

### Claude hooks do not execute

`hooks/hooks.json` is only detected today.

If you need runnable bundle hooks today, use the normal PansClaw hook-pack
layout through a supported Codex hook root or ship a native PansClaw plugin.
