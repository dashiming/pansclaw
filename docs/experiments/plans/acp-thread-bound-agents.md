...
This plan defines how PansClaw should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- PansClaw core owns ACP control plane concerns
...
PansClaw should not reimplement ACP transport internals in core.
PansClaw should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in PansClaw, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under PansClaw state dir
...
- ACP event stream is normalized to PansClaw reply chunks
...
For the first production backend (`extensions/acpx`), PansClaw and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses PansClaw `sessionKey`)
...
- PansClaw consumes ndjson events from `acpx --format json --json-strict`
