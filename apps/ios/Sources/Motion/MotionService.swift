...
final class MotionService: MotionServicing {
    func activities(params: PansClawMotionActivityParams) async throws -> PansClawMotionActivityPayload {
        ...
        let mapped: [PansClawMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return PansClawMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: PansClawPedometerParams) async throws -> PansClawPedometerPayload {
        ...
        let payload: PansClawPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = PansClawPedometerPayload(
