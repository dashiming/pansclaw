...
struct IOSGatewayChatTransport: PansClawChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> PansClawChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(PansClawChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> PansClawChatHistoryPayload {
        ...
        return try JSONDecoder().decode(PansClawChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [PansClawChatAttachmentPayload]) async throws -> PansClawChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [PansClawChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(PansClawChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(PansClawGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: PansClawGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: PansClawChatEventPayload.self)
                        {
