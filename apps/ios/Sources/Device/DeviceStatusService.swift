...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> PansClawDeviceStatusPayload {
        ...
        return PansClawDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> PansClawDeviceInfoPayload {
        ...
        return PansClawDeviceInfoPayload(
    ...
    private func batteryStatus() -> PansClawBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: PansClawBatteryState = switch device.batteryState {
        ...
        return PansClawBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> PansClawThermalStatusPayload {
        let state: PansClawThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return PansClawThermalStatusPayload(state: state)
    }

    private func storageStatus() -> PansClawStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return PansClawStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
