...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> PansClawNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> PansClawNetworkStatusPayload {
        let status: PansClawNetworkPathStatus = switch path.status {
    ...
    private static func fallbackPayload() -> PansClawNetworkStatusPayload {
        PansClawNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
