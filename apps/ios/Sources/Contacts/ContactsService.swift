...
final class ContactsService: ContactsServicing {
    ...
    func search(params: PansClawContactsSearchParams) async throws -> PansClawContactsSearchPayload {
        ...
        return PansClawContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: PansClawContactsAddParams) async throws -> PansClawContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return PansClawContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return PansClawContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> PansClawContactPayload {
        PansClawContactPayload(
