...
final class RemindersService: RemindersServicing {
    func list(params: PansClawRemindersListParams) async throws -> PansClawRemindersListPayload {
        ...
        let payload: [PansClawReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return PansClawReminderPayload(
        ...
        return PansClawRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: PansClawRemindersAddParams) async throws -> PansClawRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = PansClawReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return PansClawRemindersAddPayload(reminder: payload)
    }
