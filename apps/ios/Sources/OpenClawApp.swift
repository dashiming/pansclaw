...
@MainActor
final class PansClawAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
...
enum WatchPromptNotificationBridge {
    ...
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: PansClawWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> PansClawWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return PansClawWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "PansClaw" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [PansClawWatchAction]) -> [UNNotificationAction] {
...
@main
struct PansClawApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(PansClawAppDelegate.self) private var appDelegate
...
extension PansClawApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("PansClaw: installing uncaught exception handler")
