...
enum SessionMenuPreviewLoader {
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> PansClawSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: PansClawSessionPreviewEntry,
    ...
    private static func cache(payload: PansClawSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: PansClawChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [PansClawKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [PansClawKit.AnyCodable]) -> [PansClawChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(PansClawChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: PansClawChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: PansClawChatMessage) -> Bool {
    ...
    private static func toolNames(for message: PansClawChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: PansClawChatMessage) -> String? {
