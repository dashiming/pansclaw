...
enum PansClawLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = PansClawOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = PansClawFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        PansClawLogging.bootstrapIfNeeded()
        let label = PansClawLogging.makeLabel(subsystem: subsystem, category: category)
...
struct PansClawOSLogHandler: AppLogLevelBackedHandler {
...
struct PansClawFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = PansClawLogging.parseLabel(self.label)
