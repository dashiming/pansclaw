...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: PansClawProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: PansClawKit.AnyCodable]()) {
                $0[$1.key] = PansClawKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: PansClawProtocol.AnyCodable?) -> [String: PansClawProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: PansClawProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: PansClawKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: PansClawProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
