...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: PansClawProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: PansClawProtocol.AnyCodable] = [
            "presence": .init([PansClawProtocol.AnyCodable(entry)]),
        ]
        let payload = PansClawProtocol.AnyCodable(payloadMap)
