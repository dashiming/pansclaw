...
@MainActor
public struct PansClawChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: PansClawChatViewModel
    ...
    public init(
        viewModel: PansClawChatViewModel,
    ...
    private var visibleMessages: [PansClawChatMessage] {
        let base: [PansClawChatMessage]
    ...
    private func mergeToolResults(in messages: [PansClawChatMessage]) -> [PansClawChatMessage] {
        ...
        for message in messages {
            ...
            let merged = PansClawChatMessage(
                id: last.id,
                role: last.role,
                content: content,
    ...
    private func isToolResultMessage(_ message: PansClawChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: PansClawChatMessage) -> Bool {
    ...
    private func primaryText(in message: PansClawChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: PansClawChatMessage) -> Bool {
    ...
    private func toolCalls(in message: PansClawChatMessage) -> [PansClawChatMessageContent] {
    ...
    private func inlineToolResults(in message: PansClawChatMessage) -> [PansClawChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: PansClawChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: PansClawChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(PansClawChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
